...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const [customerInfo, setCustomerInfo] = useState<CustomerInfo>({ name: '', phone: '', email: '', address: '', city: '' });
  ...
  useEffect(() => {
    if (isMounted) localStorage.setItem('xpitienda-cart', JSON.stringify(cart));
  }, [cart, isMounted]);
  ...
  const addToCart = (product: any) => {
    setCart(prev => {
      ...
      return [...prev, { ...product, quantity: product.quantity || 1, image: product.image_url || product.image || '' }];
    });
  };
  ...
  const updateCustomerInfo = (info: Partial<CustomerInfo>) => setCustomerInfo(prev => ({ ...prev, ...info }));
  const toggleSaveData = () => setSaveCustomerData(prev => !prev);
  
  const clearCustomerInfo = () => {
    setCustomerInfo({ name: '', phone: '', email: '', address: '', city: '' });
  ...
  return (
    <CartContext.Provider value={{ cart, customerInfo, saveCustomerData, addToCart, removeFromCart, updateQuantity, clearCart, isInCart, total, subtotal, updateCustomerInfo, toggleSaveData, clearCustomerInfo, isCartOpen, setIsCartOpen }}>
      {children}
