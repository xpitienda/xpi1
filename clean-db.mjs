...
const deleteResult = await turso.execute("DELETE FROM catalog WHERE name != 'Hernan'");
console.log('Se eliminaron ' + deleteResult.rowsAffected + ' productos.');

console.log('');
console.log('Verificando base de datos actual:');
const verifyResult = await turso.execute('SELECT id, name, image_url FROM catalog');
