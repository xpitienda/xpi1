...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="bg-[#1a0a2e]/80 rounded-xl overflow-hidden border border-[#6b3fa0]/30 hover:border-xpi-green/50 transition-all group">
      {/* Image */}
      <div className="relative h-40 overflow-hidden bg-[#2d1b4e]/50">
        <img 
        ...
        />
        {product.category && (
          <span className={`absolute top-2 left-2 ${getCategoryColor(product.category)} text-white px-2 py-1 rounded-full text-xs font-semibold uppercase`}>
            {product.category}
          </span>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-xpi-green flex items-center justify-center hover:opacity-90 transition-opacity"
          >
